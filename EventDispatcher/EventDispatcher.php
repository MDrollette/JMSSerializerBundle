...
class EventDispatcher implements EventDispatcherInterface
{
    ...
    public function addSubscriber(EventSubscriberInterface $subscriber)
    {
        foreach ($subscriber->getSubscribedEvents() as $eventData) {
            if ( ! isset($eventData['name'])) {
                throw new \InvalidArgumentException(sprintf('Each event must have a "name" key.'));
            }

            $method = isset($eventData['method']) ? $eventData['method'] : self::getDefaultMethodName($eventData['name']);
            $class = isset($eventData['class']) ? strtolower($eventData['class']) : null;
            $format = isset($eventData['format']) ? $eventData['format'] : null;
            $this->listeners[$eventData['name']][] = array(array($subscriber, $method), $class, $format);
        }
    }

    public function hasListeners($name, $class, $format)
    {
        ...
        if ( ! isset($this->classListeners[$name][$loweredClass][$format])) {
            $this->classListeners[$name][$loweredClass] = $this->initializeListeners($eventName, $loweredClass, $format);
        }

        return !!$this->classListeners[$name][$loweredClass][$format];
    }
    ...
    public function dispatch($eventName, $class, $format, Event $event)
    {
        ...
        $loweredClass = strtolower($class);
        if ( ! isset($this->classListeners[$name][$loweredClass][$format])) {
            $this->classListeners[$name][$loweredClass][$format] = $this->initializeListeners($eventName, $loweredClass, $format);
        }

        foreach ($this->classListeners[$name][$loweredClass][$format] as $listener) {
